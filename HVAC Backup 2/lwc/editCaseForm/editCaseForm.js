...
import getCase from '@salesforce/apex/CreateCaseFormController.getCase';
...
export default class EditCaseForm  extends NavigationMixin(LightningElement) {
    ssduserId = Id;
    @api recordId;
    @api assetId;
        ...
        getCase({recordId:this.recordId})
                       .then(result =>{
                        ...
                        if(result!=null && result.Id!=null){
                            ...
                            this.account.Id = result.AccountId;
                            if(result.WorkOrders!=null && result.WorkOrders.length>0){
                                this.showToast('Service ticket has already been generated on this case, case is now not availble for edit.','Error'); 
                                 this.showRecordTypeScreen = false;                                  
                            }else{
                                this.handleCommAccount();
                       ...
                       // var a = result.split(";");
                        if(result.hasError==false){
                            ...
                            if(this.assetId!=null && this.assetId!=undefined && this.caseRecord.Type == 'Regular Service'){
                                ...
                                console.log('ssd 2');
                                if(this.ssdUser==true && this.caseRecord.Type!='Enquiry'){
                                    console.log('ssd');
                                    this.showToast('Successfully created the case and service ticket.','success');
                                    this.navigateToRecord(result.serviceTicket.Id);
                                    this.showSpinner = false;
