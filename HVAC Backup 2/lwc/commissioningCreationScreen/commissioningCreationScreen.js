...
import getAccountCombination from '@salesforce/apex/CommissioningCreationController.getAccountCombination';
import isServiceTerritoryAssigned  from '@salesforce/apex/CommissioningCreationController.isServiceTerritoryAssigned';
import fetchBranch from '@salesforce/apex/CommissioningCreationController.fetchBranch';
...
import fetchProductCodes from '@salesforce/apex/CommissioningCreationController.fetchProductCodes';
...
import isDealerPortalUser from '@salesforce/apex/CommissioningCreationController.isDealerPortalUser';
...
import getTotalAccountCount from '@salesforce/apex/CommissioningCreationController.getTotalAccountCount';


export default class CommissioningCreationScreen extends NavigationMixin(LightningElement) {

    productsPerPage = 10; // Rows per page
    @track currentPage = 1; // Current page
    @track totalPages = 1;
    @track totalAccCount = 0;
    ...
    @track headerLabels = [
        { FirstName: 'FirstName', LastName: 'LastName', Phone: 'Phone', Email: 'Email', OtherPhone: 'Secondary Phone No', Role__c: 'Role' }
    ];
    isNamo = false;
    existingAsset = false;
    ...
    @track existingAccountVisibility = false;
    @track branchName;
    @track branchId;
    @track showAvailableBranch = false;
    @track filteredOptionsBranch = [];
    ...
    handleNextPage() {
        if (this.currentPage < this.totalPages) {
            ...
            this.fetchAccountFromCombination(); // Fetch products for the new page
        }
    }
    ...
    fetchTotalAccountCount() {
        getTotalAccountCount({ accountName: this.searchAccountName, customerCode: this.searchCustomerCode, pincode: this.searchPincode, phoneNumber: this.searchPhoneNumber })
            .then(result => {
                this.totalAccCount = result;
                this.totalPages = Math.ceil(this.totalAccCount / this.productsPerPage);
            })
            .catch(error => {
                this.showToast('Error fetching total account count.', 'error');
    ...
    wiredObjectInfo({ error, data }) {
        if (data) {
            ...
            Object.keys(data.recordTypeInfos).forEach(key => {
                ...
                if (recordTypeInfo && recordTypeInfo.name === 'Product Sub Family') {
                    this.subFamilyRecordTypeId = recordTypeInfo.recordTypeId;
                    console.log('Record Type ID:', this.subFamilyRecordTypeId);
                }
            });
   ...
   get subFamilyFilter() {
        let criteria = [
            {
                ...
                operator: 'eq',
                value: this.subFamilyRecordTypeId // Sub Family record type ID
            }
        ];
     ...
     setFrom(event){    
        if(event.detail!=null && event.detail.recordId!=null){
            this.subfamilyId=event.detail.recordId;
        }    
        console.log('subFamilyId 1 - '+this.subfamilyId);
    }
    ...
    get productTypeOptions() {
        if (this.isDealerPortal) {
            // If isDealerPortal is true, return only the last option
            return [
                { label: "Ducted Split/Package", value: "Ducted Split" }
            ];
        }
    ...
    connectedCallback() {
        ...
        isDealerPortalUser()
            .then((result) => {
                this.isDealerPortal = result;
            })
            .catch((error) => {
                console.error('Error determining portal context', error);
            });
    ...
    handleCustomerCodeChange(event) {
        this.searchCustomerCode = event.detail.value;
    }


    // fetchAccountFromCombination() {

    //     getAccountCombination({ accountName: this.searchAccountName, pincode: this.searchPincode, phoneNumber: this.searchPhoneNumber })
    //         .then(result => {
    ...
    fetchAccountFromCombination() {
        ...
        this.isLoading = true;
        this.fetchTotalAccountCount();
        getAccountCombination({ pageSize:this.productsPerPage, pageNumber:this.currentPage, accountName: this.searchAccountName, customerCode: this.searchCustomerCode, pincode: this.searchPincode, phoneNumber: this.searchPhoneNumber })
            .then(result => {
                console.log('accountOptions --- ' + JSON.stringify(result));
    ...
    handleExistingAccount() {
        // this.existingAccountVisibility = true;
    }
            ...
            results.forEach(result => {
                if (result) {
                    ...
                    const isValidProduct = parsedData[0]?.PRODUCT.some(product =>
                        product.MODEL_CODE === this.logicalUnit.Model_Number__c);
    ...
    handleOnFocusCustomLookupProductCode(event) {
          const index = event.currentTarget.dataset.index;

          this.oduInputs[index].showAvailableCodes = true;
    }

    handleOnBlurCustomLookupProductCode(event) {
         const index = event.currentTarget.dataset.index;
        // Using setTimeout to wait a bit before hiding options so click on options can be captured
        setTimeout(() => {
    ...
    handleModelSerialNext(event) {
        ...
        try {
            ...
            // Check if model and serial number are provided based on productType
            if (this.productType === 'Chiller') {
                if (!this.modelNumber || !this.serialNumber) {
                    isValid = false;
                    errorMessage = 'Model number and serial number are required for products.';
                } else {
                    this.itemDetails.push({
                        MODEL_CODE: this.modelNumber || '',
                        SERIAL_NUMBER: this.serialNumber || ''
                    });
                }
            } else {
                // Validation for IDU inputs
                if (this.iduInputs && Array.isArray(this.iduInputs)) {
                    this.iduInputs.forEach(idu => {
                        if (!idu.modelNumber || !idu.serialNumber) {
                            isValid = false;
                            errorMessage = 'All IDU items must have both a model number and serial number.';
                        }
                        this.itemDetails.push({
                            type: 'IDU',
                            ...
                            family: '',
                            parentModelNumber: '',
                            MODEL_CODE: idu.modelNumber || '',
                            SERIAL_NUMBER: idu.serialNumber || '',
                ...
                // Validation for ODU inputs
                if (this.oduInputs && Array.isArray(this.oduInputs)) {
                    this.oduInputs.forEach(odu => {
                        if (!odu.modelNumber || !odu.serialNumber) {
                            isValid = false;
                            errorMessage = 'All ODU items must have both a model number and serial number.';
                        }
                        this.itemDetails.push({
                            type: 'ODU',
                            isLookupAssetSelected: false,
                            ...
                            family: '',
                            parentModelNumber: '',
                            MODEL_CODE: odu.modelNumber || '',
                            SERIAL_NUMBER: odu.serialNumber || '',
                            serviceTicketId: null,
                            serviceTicketNumber:'',
                            showAvailableServiceTicket: false
                        });
            ...
            // Check if itemDetails is not null and update the second page flag
            if (this.itemDetails && this.itemDetails.length > 0) {
                this.firstPage = false;
                this.secondPage = true;
            } else {
                throw new Error('Item details are empty or null.');
            }
        } catch (error) {
            console.error('Error in handleModelSerialNext:', error.message);
     ...
     handleNext(event) {
        ...
        if (this.picklistValue === 'Invoice Number' && !this.invoiceNumber) {
            ...
            if (this.picklistValue === 'Model and Serial Number' && (!this.modelNumber && !this.serialNumber)) {
         ...
         if (this.productType === 'Ducted Split' && (!this.cpId && !this.isDealerPortal)) {
            // If the picklist value is Model and Serial Number, modelNumber and serialNumber cannot be null
            this.showToast('Error', 'Please enter CP.', 'error');
            return;
        }

        if (!this.branchName) {
            // If the picklist value is Model and Serial Number, modelNumber and serialNumber cannot be null
            this.showToast('Error', 'Please enter Branch.', 'error');
            return;
        }

        if (!this.productType) {
            // If the picklist value is Model and Serial Number, modelNumber and serialNumber cannot be null
            this.showToast('Error', 'Please enter Product Type.', 'error');
            return;
        }
        if (!this.subfamilyId) {
            // If the picklist value is Invoice Number, invoiceNumber should not be null
            this.showToast('Error', 'Please enter subFamily.', 'error');
            return;
        }
          
        // ✅ Service Territory validation
        
         isServiceTerritoryAssigned({ cp: this.cpId })
            .then(result => {
                // Apex returns Boolean: true → found, false → not found
                if (!result) {
                    this.showToast(
                        'Error',
                        'Service Territory is not assigned to the CP .',
                        'error'
                    );
                    // this.showSpinner = false;
        ...
        console.log('second page', this.secondPage);
         ...
         .catch(error => {
            ...
            'Error',
                    'Could not verify Service Territory assignment.',
                    'error'
    ...
    loadBranch(searchBranch) {
        fetchBranch({ searchTerm: searchBranch})
            .then(result => {
                console.log('branch.... ' + JSON.stringify(result));
                const branchSet = new Set(result); // Using Set to store unique branch
                this.filteredOptionsBranch = Array.from(branchSet).map(branch => {
                    return { label: branch.Name, value: branch.Id };
                });
                //this.showAvailableCodes = this.filteredOptionsProductCode.length > 0;
                console.log('filteredOptionsBranch -- ' + JSON.stringify(this.filteredOptionsBranch));
            })
            .catch(error => {
                console.error('Error fetching product codes', error);
            });
    ...
    handleOnClickCustomLookupBranch(event) {
        try {

            console.log('filteredOptionsBranch on click - ', this.filteredOptionsBranch.length);
            this.showAvailableBranch = this.filteredOptionsBranch.length > 0;
        }
        catch (error) {
            console.log('error - ', error);
            console.log('error - ', error.message);
    ...
    handleOnChangeCustomLookupBranch(event) {
        const searchTermBranch = event.target.value.toLowerCase();
        this.branchName = event.target.value;
        console.log('searchTermBranch  - ', searchTermBranch);
        this.loadBranch(searchTermBranch);
    }

    handleOnFocusCustomLookupBranch() {
        this.showAvailableBranch = true;
    }

    handleOnBlurCustomLookupBranch() {
        // Using setTimeout to wait a bit before hiding options so click on options can be captured
        setTimeout(() => {
            this.showAvailableBranch = false;
        }, 300);
    }

    handleOptionClickBranch(event) {
        this.branchName = event.currentTarget.dataset.value;
        console.log(' branchname 964 - ' + this.branchName);
    }

    handleBranchChangeCustomLookup(event) {
        console.log("option clicked: ");

        this.branchName = event.currentTarget.dataset.value;
        this.branchId = event.currentTarget.dataset.id;

        console.log("option clicked: " + this.branchName);
        this.showAvailableBranch = false;
    ...
    handleBillingTypeChange(event) {
        ...
        console.log('Selected Billing Type:', this.selectedBillingType);

        if (this.selectedBillingType != 'Direct Billing') {
            this.regtypeVisibility = true;

        } else {
            this.picklistValue = 'Invoice Number';
            this.invoiceNumberScreen = true;
            this.assetDetailsScreen = true;
        }
        console.log('Selected Billing Type:', this.selectedBillingType);
    ...
    loadCPaccount(searchCP) {
        getCpAccounts({ searchTerm: this.searchCP})
            .then(result => {
                console.log('cp.... ' + JSON.stringify(result));
                const cpSet = new Set(result); // Using Set to store unique branch
                this.filteredOptionsCP = Array.from(cpSet).map(cp => {
    ...
    handleOnClickCustomLookupCP(event) {
        try {
            console.log('filteredOptionsCP on click - ', this.filteredOptionsCP.length);
    
            this.loadCPaccount(this.searchCP);
    ...
    handleOnChangeCustomLookupCP(event) {
        const searchTermCP = event.target.value.toLowerCase();
        this.searchCP = event.target.value;
        console.log('searchTermCP  - ', searchTermCP);
        this.loadCPaccount(searchTermCP);
        //this.loadServiceteretory(searchTermCP);
    ...
    handleOptionClickCP(event) {
        this.searchCP = event.currentTarget.dataset.value;
        console.log('searchCP 964 - ' + this.searchCP);
    }
    
    handleCPChangeCustomLookup(event) {
        console.log("CP option clicked: ");
    
        this.searchCP = event.currentTarget.dataset.value;
        this.cpId = event.currentTarget.dataset.id;
    
        console.log("CP option clicked: " + this.searchCP);
        this.showAvailableCP = false;
    }
