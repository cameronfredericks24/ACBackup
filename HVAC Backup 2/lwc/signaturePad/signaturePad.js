...
import getCustomerType from '@salesforce/apex/CustomerFeedbackController.getCustomerType';


export default class SignaturePad extends LightningElement {

  @api recordId;
  _enableNameSigning = false;
  ...
  checkCustomerType(){
    ...
    getCustomerType({
      ...
      .then(result => {
          ...
          if(result !=null && result === 'Commercial Customer'){
  ...
  handleCustomerCodeChange(event){
    this.customerCode = event.detail.value;

          console.log('customerCode - ', this.customerCode);

         // Dispatch custom event to parent component with input text
         const custCodeEvent = new CustomEvent('customercodechange', {
          detail: { customerCode: this.customerCode }
        });
        this.dispatchEvent(custCodeEvent);
